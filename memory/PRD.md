...
- INTEGRASI KATALOG hargasukucadang.online (2026-08): di form Tambah/Edit Sparepart ada pencarian "Cari dari hargasukucadang.online" (field Nama/Kode/Tipe). Endpoint ajax/lookup_hsc.php mem-proxy POST ke situs sumber (cURL, fallback file_get_contents) lalu parse tabel #customers via DOMDocument -> JSON {kode,nama,harga,status,tipe}. Klik hasil mengisi otomatis Kode Sparepart + Nama Barang. Butuh internet keluar + ekstensi curl/dom (aktif). Teruji: endpoint mengembalikan JSON benar; widget & pencarian jalan di browser.
- PERSISTENSI PREVIEW MARIADB (2026-08): datadir MariaDB dipindah ke /app/.mariadb_data (persisten lintas resume; /var/lib/mysql NON-persisten). Supervisor: mariadb -> /app/bengkel/start_mariadb.sh (init datadir + siapkan /run/mysqld), php-bengkel -> /app/bengkel/start_preview.sh (bebaskan port 3000 dari frontend React yang auto-start, lalu php -S :3000). User/DB: bengkel_user/bengkel_pass, db bengkel (env di php-bengkel.conf). .mariadb_data di-gitignore. CATATAN: ini khusus preview; di cPanel/XAMPP data MySQL persisten normal.
